...
def GenerateResultsForAdaboost(root, weka_jar_path):
    ...
    ea.PlotAdaboostPerIterationCurves(root + '/CreditScreeningDataset/eval_agg.credit.ada_3_ts-{0}.csv',lambda x : x['prune'] == False,root + '/CreditScreeningDataset/Plots/ada/credit.noprune.itercurves.png',[2,5,10,15,20]);
    ea.PlotAdaboostPerIterationCurves(root + '/CreditScreeningDataset/eval_agg.credit.ada_3_ts-{0}.csv',lambda x : x['prune'] == True,root + '/CreditScreeningDataset/Plots/ada/credit.prune.itercurves.png',[2,5,10,15,20]);
    #iters = [2,10,20,30,50]
    #ts = [20,30,40,50,60,70,80,90,100]
    #for _ts in ts:
...
def EvaluateBestModelsOnRealTestSet(root, weka):
    credit_model_info = {'svm':(r'i-0_t-80_ts-60/svm/cvresults/cvresults.model','60'),
                  ...
                  'knn':(r'i-0_t-80_ts-60/knn/weights-uniform_neighbors--1/weights-uniform_neighbors--1.model','80')}
    ...
    ea.ComputePerformanceOnRealTestSet(credit_model_info,root+'/CreditScreeningDataset/i-0_t-80_T-20','credit.realtest.scores.csv',weka,'+')
