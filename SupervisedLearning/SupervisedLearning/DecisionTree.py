...
def RunDecisionTrees(datasets_root_folder, weka_jar_path="C:\Program Files\Weka-3-8\weka.jar", use_arff_files=True):
    ...
    testfiles = glob.glob(
        "{0}/*.test.{1}".format(datasets_root_folder, file_extn))
    ...
    for dataset_dir in u.Get_Subdirectories(datasets_root_folder):
        ...
        while(config is not None):
            ...
            run_output_dir = u.PreparePath(
                "{0}/{1}".format(dt_root, id), is_file=False)
...
def EvaluateDecisionTrees(datasets_root_folder, params_to_keep, positive_class, evaluation_output_filename="performance.csv"):
    ...
    for directory in u.Get_Subdirectories(datasets_root_folder):
        ...
        for run_output_folder in u.Get_Subdirectories(dt_output_dir):
            ...
            p, r, f = sl.GetPrecisionRecallForWekaOutputFile(
...
def EstimatePerformanceOnCreditScreeningDataset(credit_screening_data_set_root, id, datasets_to_ignore=[], force=False):
    agg_eval_file = "{0}/eval_agg.{1}.csv".format(
        credit_screening_data_set_root, id)
    eval_files = []
    for dataset in u.Get_Subdirectories(credit_screening_data_set_root):
        ...
        RunDecisionTrees(dataset)
        EvaluateDecisionTrees(dataset, ['dataset_instance', 'test_split', 'train_split', 'random_state', 'noise_perc',
                                        'train_split_percent_used', 'imbalance_perc', 'prune', 'modelbuildtimesecs'], '+', eval_file)
    df = None
...
def EstimatePerformanceOnLetterRecognitionDataset(letter_recognition_data_set_root, id, datasets_to_ignore=[], force=False):
    agg_eval_file = "{0}/eval_agg.{1}.csv".format(
        letter_recognition_data_set_root, id)
    eval_files = []
    for dataset in u.Get_Subdirectories(letter_recognition_data_set_root):
        ...
        RunDecisionTrees(dataset)
        EvaluateDecisionTrees(dataset, ['dataset_instance', 'test_split', 'train_split', 'random_state', 'noise_perc',
                                        'train_split_percent_used', 'imbalance_perc', 'prune', 'modelbuildtimesecs'], 'v', eval_file)
    df = None
    for file in eval_files:
        d = pd.read_csv(file)
        df = pd.concat([df, d], axis=0,
...
def RunDecisionTreesOnCreditScreeningDataset(root=r"C:\Users\shkhandu\OneDrive\Gatech\Courses\ML\DataSets\CreditScreeningDataset", weka_jar_path="C:\Program Files\Weka-3-8\weka.jar"):
    ...
    keys_to_keep = ['dataset_instance', 'test_split', 'train_split', 'random_state',
                    'noise_perc', 'train_split_percent_used', 'imbalance_perc', 'prune', 'modelbuildtimesecs']

    def classifier_fn(x): return RunDecisionTrees(
        x, weka_jar_path=weka_jar_path)
    id = "credit.dt_2_all"
    algo_folder = 'dt'
...
def RunDecisionTreesOnVowelRecognitionDataset(root=r"C:\Users\shkhandu\OneDrive\Gatech\Courses\ML\DataSets\LetterRecognition", weka_jar_path="C:\Program Files\Weka-3-8\weka.jar"):
    ...
    def classifier_fn(x): return RunDecisionTrees(x, weka_jar_path)
    id = "vowel.dt_1_all"
    algo_folder = 'dt'
    force_computation = True
    RunNEvaluateExperimentsOnDataSet(
